...
public class SlideShowFX extends Application {
	...
	class ScannerLoader extends Thread implements FileVisitor<Path> {
		...
		private String root = "/Users/lliu/Pictures";
		...
		public void run() {
			...
			try {
				...
				Logger.getLogger(SlideShowFX.class.getName()).log(Level.SEVERE,
						null, ex);
			} finally {
				complete = true;
			}
		...
		public FileVisitResult visitFile(Path t, BasicFileAttributes bfa)
				throws IOException {
			try {
				System.out.println("file: " + t.toUri().toString());
				Image image = new Image(t.toUri().toString(), width, height,
						true, true, false);
	...
	public void startImage(Image image) {
		...
		if (current != null) {
			...
			dropout.setToX(0.75);
			dropout.setToY(0.75);
			st = new SequentialTransition(new ParallelTransition(fadein,
					dropout), delay);
		} else {
	...
	Image getNextImage() {
		...
		try {
			...
			Logger.getLogger(SlideShowFX.class.getName())
